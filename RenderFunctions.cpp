...
void draw_empty_cell(SDL_Surface* surface, std::vector<std::vector<Uint32>> grid, int cell_size, int x_offset, int y_offset, int x_index, int y_index) {

    for (int k = 0; k < 3; k++) {
        draw_square(surface, x_index * cell_size + x_offset + k * 2, y_index * cell_size + y_offset + k * 2, cell_size - k * 4, get_color(200 - k * 20, 200 - k * 20, 200 - k * 20));
    }

}

void draw_grid(SDL_Surface* surface, int x_offset, int y_offset, int cell_size, std::vector<std::vector<Uint32>> grid, bool symmetry_mode) {
    for (int i = 0; i < grid[0].size(); i++) {
        for (int j = 0; j < grid.size(); j++) {

            if (grid[j][i] == 100000000) {
                draw_empty_cell(surface, grid, cell_size, x_offset, y_offset, j, i);
            }
            else {
                draw_square(surface, x_offset + (j * cell_size), y_offset + (i * cell_size), cell_size, grid[j][i]);
            }
        }
    }
}

void draw_grid_lines(SDL_Surface* surface, int x_offset, int y_offset, int cell_size) {
    for (int i = 0; i < 17; i++) {
        draw_thick_line(surface, x_offset + (i * cell_size), y_offset, x_offset + (i * cell_size), y_offset + (cell_size * 16), 1, 0xFFFFFF);
        draw_thick_line(surface, x_offset, y_offset + (i * cell_size), x_offset + (cell_size * 16), y_offset + (i * cell_size), 1, 0xFFFFFF);
    }
}
